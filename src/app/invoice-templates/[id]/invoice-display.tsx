...
import {
  Autocomplete,
  AutocompleteItem, Button, Card, CardBody, CardHeader, DatePicker, Input,
  Table,
  TableBody,
  TableCell,
  TableColumn,
              ...
              {invoice.lineItems.flatMap((lineItem) => {
                return [(
                  <TableRow key={lineItem.id}>
                    <TableCell>
                      <Button isIconOnly variant="light" color="danger" onPress={() => removeLineItem(lineItem.id).then(() => router.refresh())}>
                        <FaTrash/>
                      </Button>
                    </TableCell>
